...
void CurrentPulseON()
{
  ...
  while (value < 680);                  // ~3V this voltage means that the current pulse is ready
 //delay(10);
...
void ModeDisc(uint8_t discNumber)
{
  ...
  delayMicroseconds(2000);                             // Flip-disc required 1ms current pulse to flip
...
{
  SPI.transfer(discNumber);   // Send data to the controller
}
